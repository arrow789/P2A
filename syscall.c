...
syscall(void)
{
  ...
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    ...
    // 5 calls doesn't count
    if (num != 22 && num != 23 && num != 1 && num != 7 && num != 12) {
      // check the return value
      if (curproc->tf->eax != -1) {
        curproc->countergood = curproc->countergood + 1;
      }
    ...
    curproc->tf->eax = -1;
              ...
              // count all sys call
              // if (tf->eax != 22 && tf->eax != 23 && tf->eax != 1 && tf->eax != 7 && tf->eax != 12)
              // {
